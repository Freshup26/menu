...
import {
  getDatabase, ref as dbRef, get, set, push, update, remove, child
...
function toArray(obj){if(!obj)return[];return Object.entries(obj).map(([id,v])=>({id,...v}));}

// ============================================================
//  Seed
// ============================================================
async function ensureSeed() {
  const snap = await get(dbRef(db, 'menus'));
  if (!snap.exists()) {
    const m1 = push(dbRef(db, 'menus'));
    const m2 = push(dbRef(db, 'menus'));
    await update(dbRef(db), {
      [`menus/${m1.key}`]: { slug:'cafe',       name:'منيو المقهى', sort:1, createdAt: Date.now() },
      [`menus/${m2.key}`]: { slug:'restaurant', name:'منيو المطعم', sort:2, createdAt: Date.now() },
    });
  }
}
...
async function loadMenus() {
  const snap = await get(dbRef(db, 'menus'));
  state.menus = toArray(snap.val()).sort((a,b) => (a.sort||0) - (b.sort||0));
  renderMenus();
  if (state.menus.length && !state.currentMenuId) {
...
$('#addMenuBtn').addEventListener('click', () => {
  openPrompt({
    ...
    onSubmit: async ({ name, slug }) => {
      ...
      if (state.menus.find(m => m.slug === slug)) throw new Error('هذا الـ slug موجود مسبقاً');
      const sort = (Math.max(0, ...state.menus.map(m => m.sort || 0)) || 0) + 1;
      const newRef = push(dbRef(db, 'menus'));
      await set(newRef, { name, slug, sort, createdAt: Date.now() });
...
$('#renameMenu').addEventListener('click', () => {
  ...
  openPrompt({
    ...
    onSubmit: async ({ name }) => {
      await update(dbRef(db, `menus/${menu.id}`), { name });
      toast('تم التعديل');
      await loadMenus();
    }
  });
...
async function cascadeDeleteMenu(menuId) {
  const cats = state.categories.filter(c => c.menuId === menuId);
  for (const c of cats) await cascadeDeleteCategory(c.id);
  await remove(dbRef(db, `menus/${menuId}`));
}
...
async function loadWorkspace() {
  ...
  const [catSnap, subSnap, prodSnap] = await Promise.all([
    get(dbRef(db, 'categories')),
    get(dbRef(db, 'subcategories')),
    get(dbRef(db, 'products')),
  ]);

  const allCats = toArray(catSnap.val());
  state.categories = allCats
    .filter(c => c.menuId === state.currentMenuId)
    .sort((a,b) => (a.sort||0) - (b.sort||0));

  const catIds = new Set(state.categories.map(c => c.id));
  state.subcategories = toArray(subSnap.val())
    .filter(s => catIds.has(s.categoryId))
    .sort((a,b) => (a.sort||0) - (b.sort||0));
  state.products = toArray(prodSnap.val())
...
$('#addCat').addEventListener('click', () => {
  openPrompt({
    ...
    onSubmit: async ({ name }) => {
      ...
      await set(newRef, { menuId: state.currentMenuId, name, sort });
...
function renameCategory(catId) {
  ...
  openPrompt({
    ...
    onSubmit: async ({ name }) => {
      await update(dbRef(db, `categories/${catId}`), { name });
      toast('تم التعديل');
      await loadWorkspace();
    }
  });
...
async function cascadeDeleteCategory(catId) {
  const catProds = state.products.filter(p => p.categoryId === catId);
  for (const p of catProds) {
    if (p.imagePath) {
      try { await deleteObject(sRef(storage, p.imagePath)); } catch {}
    }
    await remove(dbRef(db, `products/${p.id}`));
  }
  const catSubs = state.subcategories.filter(s => s.categoryId === catId);
  for (const s of catSubs) await remove(dbRef(db, `subcategories/${s.id}`));
...
function addSubcategory(catId) {
  openPrompt({
    ...
    onSubmit: async ({ name }) => {
      ...
      const newRef = push(dbRef(db, 'subcategories'));
      await set(newRef, { categoryId: catId, name, sort });
      toast('تم الإضافة');
      await loadWorkspace();
    }
  });
...
async function deleteSubcategory(subId) {
  ...
  openConfirm(`حذف القسم الفرعي "${s.name}"؟`, async () => {
    // unlink products
    const updates = {};
    state.products.filter(p => p.subcategoryId === subId).forEach(p => {
      updates[`products/${p.id}/subcategoryId`] = null;
    });
    updates[`subcategories/${subId}`] = null;
...
prodForm.addEventListener('submit', async e => {
  ...
  try {
    ...
    if (id) {
      await update(dbRef(db, `products/${id}`), payload);
      ...
      await set(newRef, { ...payload, sort, createdAt: Date.now() });
...
function deleteProduct(prodId) {
  ...
  openConfirm(`حذف المنتج "${p.name}"؟`, async () => {
    ...
    await remove(dbRef(db, `products/${prodId}`));
    toast('تم الحذف');
    await loadWorkspace();
  });
}
